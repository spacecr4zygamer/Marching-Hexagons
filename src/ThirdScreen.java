...
public class ThirdScreen extends JPanel {
    ...
    public void paint(Graphics g) {
        ...
        if (alpha==1||alpha==0) { toggle = !toggle; }
...
//        }
        ...
        for (int casenumber = 0; casenumber < data2.length; casenumber++) {

            g2d.setColor(Color.BLUE);
            //int[] verticestodraw = data[getIndexNumber(1,1,1,0,0,0)];
            int[] verticestodraw = data2[casenumber];
            //System.out.println("Vertices: "+verticestodraw.length);
            Point middlepos = converttoHexagongrid(xoff - 1, yoff - 1);
            Point[] points = getVerticePositions(middlepos);
...
//                    lerppoint(left, upleft, 0.5)
            ...
            //Point[] connectionPoints = getConnectionPoints(middlepos);
            for (int i = 3; i <= verticestodraw.length; i += 4) {
                //System.out.println(i);
                Point a_before = verts[verticestodraw[i - 3]-1],
                        a_next = verts[verticestodraw[i - 2]-1],
                        b_before = verts[verticestodraw[i - 1]-1],
                        b_next = verts[verticestodraw[i]-1];
                /*g2d.setColor(Color.yellow);
                g2d.fillOval(a_before.x - 7, a_before.y - 7, 14, 14);
                g2d.fillOval(a_next.x - 7, a_next.y - 7, 14, 14);
                g2d.fillOval(b_before.x - 7, b_before.y - 7, 14, 14);
                g2d.fillOval(b_next.x - 7, b_next.y - 7, 14, 14);*/
                //Point a = connectionPoints[verticestodraw[i-1]-1], b = connectionPoints[verticestodraw[i]-1];
                g2d.setColor(Color.green);
                Point a = lerppoint(a_before, a_next, alpha), b = lerppoint(b_before, b_next, alpha);
                g2d.fillOval(a.x - 5, a.y - 5, 10, 10);
                g2d.fillOval(b.x - 5, b.y - 5, 10, 10);
                g2d.drawLine(a.x, a.y, b.x, b.y);
            }
